 ...
 * 运行：node scripts/bench/bench-cli.mjs [技能条数=50]
 */
...
out(`scan（${COUNT} 目录扫描+schema 校验）: ${(idx0 - t0).toFixed(1)} ms`);
out(`索引建库+持久化: ${(t1 - idx0).toFixed(1)} ms`);
out(
  `检索 100 次均值: ${mean.toFixed(2)} ms · 中位 ${median.toFixed(2)} ms · P95 ${timings[Math.floor(timings.length * 0.95)]?.toFixed(2)} ms`,
);
out(`装配（loadDetail 单次，含首部剥离+mustache）: ${assemble.toFixed(1)} ms`);
out(`对账路径: 占位（reconcile 适配器 P2 2.3 落地后启用）`);
