...
| 面 | 选择 | 备注 |
|---|---|---|
| 运行时 | **Bun-first · Node-safe**（bun install/run/test；业务代码仅标准 Node API，node:22 一键回退，CI 双跑冒烟） | [运行时决议](../decisions/2026-08-27-runtime-bun-first.md) |
| 语言 | TypeScript strict 全仓（bun 直跑，免编译层） | 包间仅经 protocol 类型耦合 |
| HTTP | Fastify + zod 边界校验 | 外部输入一律 schema 验证 |
| DB 访问 | Kysely（类型化 SQL） | 无重型 ORM |
| 迁移 | node-pg-migrate | DDL 以 architecture 本节为准 |
