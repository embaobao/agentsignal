...
- **P3 动态技能管理（[dynamic-skill-management 提案](openspec/changes/dynamic-skill-management/proposal.md) · 已批准 2026-09-08）**：skill-engine Phase 3 预留的「服务端发布机制」另立提案落地——订阅落库 × 回流闭环（订阅信号 pull 式同步落本地库 · use --install 单条装载 · verify 回流镜像 · 更新链/失效/容量治理）。红线：不新增用户命令与 MCP 工具、无常驻进程、零触碰 apps/api。**P0–P3 已全部完成（2026-09-09，CLI 98 用例 + verify 全绿）**；P4 文档随行已落（user-manual §1.6 · local-engine §10.5 · admin-guide §6.1 · SKILL verify 增量）。**发版顺序修订（站长令 2026-09-09）：4.2a 本地自管理全链验证（本地起 api + register 本地账号 + 订阅同步 + verify 回传聚合）为 npm 发版硬门 → 发 0.5.0 → 服务端生产部署押后不阻塞**。剩余：4.2 端到端 → 4.3 changeset 发版；开放问题三项按建议值执行待站长裁决。夜间自动化实施中（每小时触发轮，仅 23:00–09:00 开发）。
