...
> **定位扩展（2026-09-11 起，[决议](docs/decisions/2026-09-11-local-capability-plane.md) D1/D2）**：**经验层 + 本地能力面**——经验层管「经验从哪来、有没有效、用得怎么样、值不值得传下去」（差异位）；本地能力面管「能力怎么装到这台机器的各个 Agent 上」（不重造装载层：APM/native/pi/dsh 作**可插拔适配器**，降级三档 L1 原生→L2 按需→L3 兜底）
...
当前阶段：**本地能力面（[local-capability-plane](openspec/changes/local-capability-plane/proposal.md) · 决议签发 2026-09-11，四夜流水线 22/59 · P0 全勾待 G-A 站长抽验 · P1 1.1–1.5+1.7 契约落地（types/caps 门控/ledger 归属账本/lock 归属锁/broker 选择器/降级矩阵夹具——1.6 apm 适配器待认领）· P5 全章 5.1–5.5（三计数+验证体检+S2/S3 场景）· C1–C6 基建全件）**：定位扩展 D1「经验层 + 本地能力面」——装载层不重造（D2），APM/native/pi/dsh 作可插拔适配器；夜间七席流水线执行（23:00–08:00 免额窗口，写轨串行/审查并行/06:30 汇总定稿）；local-content-model / mcp-service-exposure 已归并被吸收（D9），host-matrix-alignment / host-integration 保留、落地归入本线 P2/P3（其自身已 11/31：Hermes 四件 + skills 落盘 + R5-a 整改）；apps/api 全程零触碰。三链路已上线 npm（@agentssignal/* 0.5.0 lockstep，2026-09-10 发版）并公网全通（agentsignal.netlify.app + Neon PG，deployment §9.1）；ux-foundation Phase 0–2 + 5 主体已实施（2026-09-02 账实对照确认）。**本地技能引擎（[skill-engine](openspec/changes/skill-engine/proposal.md)，2026-09-03 落地 Phase 1 + Phase 2 主体）**：用户面四个命令 init（一条命令接入：向导→接线→完成）/ mcp / status / uninstall；MCP 唯一 server = `agentsignal mcp`，工具面 9 = 5 平台 + 3 本地 + open_setup（[mcp-sdk-consolidation](docs/decisions/2026-09-02-mcp-sdk-consolidation.md)）；本地管理界面 = 单文件 HTML 零外部请求（真源 [management-ui-spec-v1](docs/design/management-ui-spec-v1.md)）；「技能（内部形式）」不进用户命令面/文案（[skill-envelope](docs/decisions/2026-09-02-skill-envelope.md)）；`@agentsignal/mcp` 兼容壳已随包整体移除（未发版无存量用户，见决议修订注记）。零触碰 apps/api。**Payload CMS 已结案否决**（standardize-node-postgres 决议 D5）。身份模型 Q1–Q7 + 超管已于 2026-09-01 定档（1:N · 先注册后绑定 · 双层 /me · 5 agent 上限 · 结构化 verdict · 公开聚合 · 反馈需身份 · 超管=站长），**不再开放讨论**。

剩余（2026-09-11 更新；盘查明细见 [台账对照节](docs/design/implementation-tasks.md)）：

- **当前主线 · 本地能力面（[local-capability-plane](openspec/changes/local-capability-plane/proposal.md) · 决议已签发 2026-09-11 · 四夜流水线 22/59）**：**P0 产物规范化全勾（0.1–0.4 落地 · 0.5 G-A 门请求已发等站长抽验）** → **P1 适配器骨架 1.1–1.5 全落（types/caps 门控/ledger 归属账本/lock 归属锁/broker 选择器——三规则基建齐）** → **P5 经验层收口 5.1–5.4 全落（verify_target/origin/三计数分立/status 验证体检——S2 缺口从数据到呈现贯通）** → P2 native 装载 → P3 长尾宿主+插件双轨 → P4/P7/P8 互操作/模型接入/运行时 + 轨道 C 验证基建（**C1 场景十六壳 + C2 适配器契约套件（gate/lock 钩子已接真函数）+ C3 台账对账 + C6 汇总席脚本全 ✅**，C4/C5 待做）。夜间七席流水线（写轨 A/B/C 串行 · 审查三席 05:00 并行 · 汇总席 06:30 定稿）；五道门 G-A/G-B/G-C/G-E/G-F 全部等站长真机。**host-matrix-alignment / host-integration 保留，落地归入本线 P2/P3**（D9；host-matrix 自身 14/31——Hermes 1.1–1.8 夜间面全清（types/skills 落盘/SOUL.md 块/hook 双写/事件映射/兜底守卫/测试收口）+ 2.4/2.5 注记 + R5-a 整改，只剩 1.9 真机走查）；P6 记忆层独立立项，P0–P8 后站长决。
...
- **P3 动态技能管理（[dynamic-skill-management 提案](openspec/changes/dynamic-skill-management/proposal.md) · 已批准 2026-09-08）**：skill-engine Phase 3 预留的「服务端发布机制」另立提案落地——订阅落库 × 回流闭环（订阅信号 pull 式同步落本地库 · use --install 单条装载 · verify 回流镜像 · 更新链/失效/容量治理）。红线：不新增用户命令与 MCP 工具、无常驻进程、零触碰 apps/api。**P0–P3 已全部完成（2026-09-09，CLI 98 用例 + verify 全绿）**；P4 已全部完成（2026-09-10：4.1 文档随行 · 4.2a 本地自管理全链验证硬门通过（隔离栈实测，连带修 3 真 bug：凭证路径硬编码 AGENTSIGNAL_CLIENT_DIR/大写 ULID 落库归一/mirror 漏归一）· 4.2b e2e 链路4（23/0）· **4.3 已发版 @agentssignal/* 0.5.0 lockstep（npm 实查确认）**）。**发版顺序修订（站长令 2026-09-09）：4.2a 本地自管理全链验证为 npm 发版硬门 → 已发 0.5.0 → 服务端生产部署押后不阻塞**。开放问题三项按建议值执行待站长裁决；pack-verify 沙箱终验待站长自有终端跑一次（工具环境 FS 异常记录见 tasks.md 4.2b）。自动化已随提案完成停用。
