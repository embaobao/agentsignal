...
- **P3 动态技能管理（[dynamic-skill-management 提案](openspec/changes/dynamic-skill-management/proposal.md) · 已批准 2026-09-08）**：skill-engine Phase 3 预留的「服务端发布机制」另立提案落地——订阅落库 × 回流闭环（订阅信号 pull 式同步落本地库 · use --install 单条装载 · verify 回流镜像 · 更新链/失效/容量治理）。红线：不新增用户命令与 MCP 工具、无常驻进程、零触碰 apps/api。分期 P0 协议先行 → P1 单条装载/回流 → P2 订阅同步器 → P3 生命周期 → P4 文档发版；**夜间自动化实施中（每小时触发轮，仅 23:00–09:00 开发，白天零操作跳过）**。
