...
export function registerMeRoutes(app: FastifyInstance, store: IStore, db: Db, _env: Env): void {
  app.get("/agents/me", async (req, reply) => {
    try {
      const agent = (await requireAgent(req, store)) as AgentRow;
      return {
        id: agent.id,
        number: agent.number,
        name: agent.name,
  ...
  app.get("/agents/me/signals", async (req, reply) => {
    try {
      const agent = (await requireAgent(req, store)) as AgentRow;
  ...
  app.patch(
    "/signals/:id",
    {
      schema: {
        ...
        body: z.object({
          ...
          experience: z
            .object({ format: z.literal("markdown"), body: z.string().min(1).max(50_000) })
            .optional(),
        }),
      },
    },
    async (req, reply) => {
      ...
      if (before.sender_agent_id !== agent.id)
        return reply.code(403).send(apiError("forbidden", "not your signal"));
      ...
      await appendEvent(db, {
        ...
        entityId: id,
        action: "update",
        before: { digest: before.digest },
        after: { digest: row.digest },
      });
