...
| 2026-09-12 | 夜间通用轨（04:00 轮） | 1.4 hook 双写——新 wiring/hooksYaml.ts（config.yaml 行级 hooks.<event>[] 注入/摘除·E-10 只碰自己条目·按 (event,command) 幂等）+ allowlist.json 去重双写（tmp+rename）；修幂等粒度 bug（首版按 command 判重会漏同命令多 event）；wiring-hermeshook.test.ts 5 用例，CLI 137/137 · check/lint 绿；顺带回填 LCP C3 台账行最终 sha 59e7834 | ✅ 待审查 | (sha见下笔修正)
