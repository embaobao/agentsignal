...
- [x] 1.2 skills 落盘：`$hermesHome/skills/<name>/SKILL.md`（目录式），复用既有 skill 写入器（wiring/hostSkills.ts：产物渲染复用 renderArtifactSkillMd·宿主侧仅 SKILL.md 零泄漏·id 白名单 fail-fast·卸载只删自己条目零残留；init 全链接线随 1.9 真机走查验证）
- [x] 1.3 **SOUL.md 块注入**：标记段 `<!-- agentsignal:rules:start/end -->`；文件不存在则创建，存在则追加；重复注入不产生第二块；摘除只删标记段 —— 单测覆盖「用户原有内容完好」（已接线 wireMcp/unwireMcp extra 分支：hermes rules 兜底 = SOUL.md 块注入，零垃圾断言升级为「仅 SOUL.md 无 MCP 文件」）
...
- [x] 1.7 兜底：config.yaml 解析失败 → 跳过 hooks 只发 skill，不 fail 整个 init（异常落 stderr）（writeExtra hermes 分支整体守卫：SOUL.md/hooksYaml 任一落盘失败 → stderr 可见告警 + extra=none 降级只发 skill；hook 命令带 --event SessionStart 事件映射；R5-b 空行压缩越界于次日修复轮真收敛（去全文件压缩，E-10 保守））
- [x] 1.8 测试：`packages/cli/test/` 扩展 `AGENTSIGNAL_HOME` 夹具 —— Hermes 探测 / 写入（含 SOUL.md 块 + 双写 + 无 MCP 文件）/ 摘除不残留 三断言 + 专项 3 条（见 design.md §五）（由 1.1–1.7 各单元测试分摊覆盖齐：探测/写入/摘除三断言 = wiring-hermes+hostskill+hermeshook+soul 四文件 19 用例；§五 专项 3 条全在；1.8 本笔仅补「探测命中」用例闭口）
...
- [x] 2.4 清偿既存漂移：`hosts.ts` 文件头注释提到 Trae 但 `HostId` 无 trae —— 二选一（补 trae 或删注释）**【需站长裁决 · 阻塞项：夜间轮自动选「删注释」保守执行并留档，站长可翻案】**（2026-09-12 夜已按授权选「删注释」：头注释已重写为当下事实，Trae 不再被提及；站长可翻案补 trae）
- [x] 2.5 文件头注释写入 **D7 不猜 MCP 纪律**（抄 TeamAI 原文口径：没把握的宿主一律不写 MCP，只发 skill + rules 一行兜底）
