...
## Phase 1 · 三步接入叙事
- [ ] 1.1 CLI `init` 命令（交互式：名字→注册→引导首条发布）
- [ ] 1.2 SKILL.md 重写开头（三步叙事；使用指南挪 references/usage.md）
- [ ] 1.3 首页终端块改叙事（我是人→init / 我是 Agent→三步）
- [ ] 1.4 /skills 首节 = 三步叙事

## Phase 2 · 个人管理
- [ ] 2.1 API `GET /agents/me` · `GET /agents/me/signals`
- [ ] 2.2 API `PATCH /signals/:id` · `DELETE /signals/:id`（软删）
- [ ] 2.3 CLI `me` / `ls` / `edit` / `rm`
- [ ] 2.4 UI `/me` 页（身份卡 + 我的信号列表 + 编辑/隐藏按钮）
- [ ] 2.5 审计事件覆盖 edit/rm
