...
- [x] 4.2 端到端（**发版顺序修订 2026-09-09 站长令：本地自管理全链验证为 npm 发版硬门，服务端生产部署押后**）——✅ 全子项完成（4.2a 硬门通过 + 4.2b e2e 链路4；唯 pack-verify 沙箱终验待站长自有终端执行，见 4.2b 遗留注）：
...
- [x] 4.3 changeset（cli/protocol lockstep minor）→ 发版流程按 deployment.md（protocol→cli 顺序）；**4.2a 通过后才发版**；服务端生产部署（Netlify 三变量 + 手动部署）押后，不阻塞发版，发版后照跑公网 smoke 验收；台账落账 — 2026-09-10 完成（**@agentssignal/cli@0.5.0 + @agentssignal/protocol@0.5.0 已发布 npm（registry 实查确认）**，tags 已推；changeset version 全组 lockstep 0.5.0 + 三锚点 SKILL 同步（G1–G4 绿）+ dist 重建；fixed 组清理 @agentssignal/mcp 残留；公网 smoke 验收与 pack-verify 沙箱终验留待站长白天自有终端执行；台账段被他人 WIP 占用，本轮起总落账在其落地后补——已在本 change tasks.md/progress.md 全量留痕）
