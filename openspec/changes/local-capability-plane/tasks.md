...
### 0.1 夜间流水线（四席 · 顺序执行 · 免额窗口内）

| 席 | 会话 | 时间 | 职责 | 硬边界 |
|---|---|---|---|---|
| 1 | zcode 主轨开发 | 23:00 | **轨道 A**：取任务→实现→测试→commit | 只做轨道 A 任务 |
| 2 | zcode-b 副轨开发 | 01:30 | **轨道 B**：同上 | 只做轨道 B 任务 |
| 3 | zcode-review 审查 | 04:00 | 审两轨夜间 diff vs DoD+红线 → 审查报告 | **不改码；可驳回** |
| 4 | zcode-fix 修复验证 | 06:00 | 处理驳回与验证失败 + 全量 verify | 不可解→升级报告 |
