...
| 2026-09-14 | 夜间通用轨（00:00 轮） | 1.5 适配器选择器——capability/broker.ts（selectAdapter：preferred 置顶→native 加权→probe 过滤逐个换不报错→全不可用 undefined 走降级 D10；纯逻辑零副作用）；capability-broker.test.ts 6 用例，CLI 176/176 · check/lint 绿 | ✅ 待审查 | (sha见下笔)
