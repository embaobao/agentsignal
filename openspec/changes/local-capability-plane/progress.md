...
| 2026-09-15 | 夜间通用轨（04:00 轮） | 4.2 apm.yml 导入器——export.ts importApm（4.1 输出契约行级解析·首部校验 name=目录名·origin=apm-import 溯源·私有层安全默认重建不回转·坏条目跳过）；S10 往返正文逐字无损断言；CLI 199/199 · check/lint 净 | ✅ 待审查 | `bc8daa1`
| 2026-09-15 | 夜间通用轨（06:00 修复轮） | 审查移交清零：1.6/1.7 勘误（降级夹具实属 1.7 已勾·1.6=apm 适配器保持待认领·d3966d7 误挂注记）· 补 importApm skipped 分支用例（DoD 3 用例回账）· C2 加「ladder≠L1⇒skipped/warnings 非空」结构断言（多档盲区闭合）· check-ledger 重复 return 清理；残留 1 项=d3966d7 历史误挂（勘误注记在案，与 0.1-0.4 同类豁免）；CLI 200/200 · check/lint 绿 | ✅ 待审查 | `ac329f3`
| 2026-09-15 | 席7·汇总修复（06:20-07:25 槽） | 三报告已入库（06:00 修复轮）· 移交全清零（1.6/1.7 勘误+skipped 用例+C2 结构断言+ledger 清理）· AGENTS.md 两节定稿（**四夜 22/59**·host-matrix 14/31 实盘数·P4 4.1/4.2 入册）· 全量门禁 db:up 仍阻塞（Docker 连续 31 轮）→ 替代口径 check+lint+CLI 200/200+todo14+test:ui 37/37 绿·verify 全链留痕；tasks.md 定稿 | ✅ 定稿 | (sha见git log)
| 2026-09-15 | 席1·主轨开发（轨道A） | 1.6 APM 适配器——capability/adapters/apm.ts（probe spawn 探测缺席 reason/DECLARED_CAPS 不含 remove/remove 门控抛 UnsupportedCapability/hosts 五宿主·deploy/reconcile 占位注记待 G-B）；capability-apm.test.ts 4 用例（PATH 沙箱 fake apm 取版本/缺席降级/规则一）；CLI 204/204 · check/lint 绿 | ✅ 待审查 | `2113efc`
| 2026-09-16 | 夜间通用轨（00:00 轮） | 4.3 扩展 JSON——export/import 双侧 .apm/extensions.json5（导出原始私有字段保真·导入恢复升级 4.2 安全默认·标准产物仍零泄漏·无扩展文件向后兼容）；S10 往返断言升级为恢复语义；踩坑：python replace 对 biome 折行文本连续未命中致写回段漏改（隔离 repro 定位）；⚠️ apps/api 有他人 WIP（audit-restore 后续，admin.ts snapshotBefore TS 错+4 文件未提交）——按协议不碰，全仓 check/lint 含 apps/api 暂红，域内门改限定 packages/+scripts/ 口径；CLI 207/207 · 限定域 lint 净 | ✅ 待审查 | `f7f5cf9`
| 2026-09-16 | 席2·副轨开发（轨道B） | 4.4 往返收口——往返断言随 4.1/4.2/4.3 落位全绿（确定性/零泄漏/正文逐字/私有增强恢复四线）；user-manual 用法按 C5 口径登记 0.6.0 随行预留（清单补第 5 项）；**P4 互操作全章闭环（4.1–4.4）**；CLI 207/207 · 限定域门禁绿 | ✅ 待审查 | `d0cd381`
| 2026-09-16 | 夜间通用轨（02:00 轮） | 7.1 供应商注册表——capability/providers.ts（ProviderSchema·内置国产端点打底 GLM/DeepSeek/Moonshot·用户区 providers.json5 覆盖/追加·fail-soft 收集·token 永不进注册表）；capability-providers.test.ts 4 用例；CLI 211/211 · 限定域门禁绿 | ✅ 待审查 | `c88b86d`
| 2026-09-16 | 席3·基建轨（支援轨） | C2 盲区加固（审查席 5 上夜建议落地）——runAdapterContract deploy 遍历全部声明宿主（原仅 hosts[0]）：多档降级 fake 三宿主逐档过闸（L1 直写/L2、L3 skipped reason+非空结构断言全部在套件内生效）；contract-adapter 增遍历自证用例；CLI 212/212 · 限定域门禁净 | ✅ 待审查 | `a1896b7`
