...
| 2026-09-15 | 夜间通用轨（04:00 轮） | 4.2 apm.yml 导入器——export.ts importApm（4.1 输出契约行级解析·首部校验 name=目录名·origin=apm-import 溯源·私有层安全默认重建不回转·坏条目跳过）；S10 往返正文逐字无损断言；CLI 199/199 · check/lint 净 | ✅ 待审查 | (sha见下笔)
